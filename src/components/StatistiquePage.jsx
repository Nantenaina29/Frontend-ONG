...
export default function StatistiquePage() {
  ...
  const stats = useMemo(() => {
    ...
    // Fiarovana: raha sendra tsy array ny data
    const fData = Array.isArray(data.formations) ? data.formations : [];
    const mData = Array.isArray(data.membres) ? data.membres : [];
    const gsData = Array.isArray(data.gs) ? data.gs : [];
    const rData = Array.isArray(data.responsables) ? data.responsables : [];
    ...
    const gsCounts = gsData.reduce((acc, curr) => {
        const d = curr?.dateCreation || curr?.DateCreation || curr?.created_at;
    ...
    return { ageM, gsByYear, respData, statsTaranja, totalM: fData.length, totalAutonome };
  }, [data]);
  ...
  return (
    <div className="stat-page-container" style={{ padding: '20px', backgroundColor: '#f8f9fc' }}> 
      <h2 style={{ textAlign: 'center', marginBottom: '30px', fontWeight: 'bold' }}>📊 Tableau de Bord Statistiques</h2>
      <div className="stat-grid-layout" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '25px' }}>
        
        <StatSection title="1. DÉMOGRAPHIE" color="#4e73df">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={stats.ageM}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" /><YAxis /><Tooltip /><Bar dataKey="valeur" fill="#4e73df" radius={[5, 5, 0, 0]} /></BarChart>
          </ResponsiveContainer>
        </StatSection>

        <StatSection title="2. CRÉATIONS GS" color="#1cc88a">
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={stats.gsByYear}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="year" /><YAxis /><Tooltip /><Bar dataKey="valeur" fill="#1cc88a" radius={[5, 5, 0, 0]} /></BarChart>
          </ResponsiveContainer>
        </StatSection>

        <StatSection title="3. RESPONSABILITÉS" color="#f6c23e">
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
        ...
        <StatSection title="4. IMPACT & AUTONOMIE" color="#e74a3b">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stats.statsTaranja}><XAxis dataKey="name" fontSize={9} /><Tooltip /><Bar dataKey="valeur" fill="#e74a3b" /></BarChart>
            </ResponsiveContainer>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
